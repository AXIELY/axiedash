-- Fix: perform_spin referenced `created_at` on game_logs, but the column is `played_at`
-- This caused a PostgreSQL exception that aborted every V2 spin.
...
BEGIN
  ...
  -- Server-side weighted prize selection (exclude strong prizes already won today)
  SELECT ARRAY(
    SELECT (sr2.prize_id)
  ...
  -- Legacy game_logs entry (played_at is the correct column name, not created_at)
