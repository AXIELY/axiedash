/*
# Fix Probability Resolution — Exclude Disabled Prizes from Random Bucket

## Problem
The random bucket (0-9999) includes ALL prizes in the cumulative sum, including
disabled ones. When a disabled prize is selected, it falls back to `points-1`.
With the current config (points-1=63.86%, libyana=28.10% disabled, tiktok=6.40%
disabled, grand=1% locked, prize_1783884291700=0.64% locked), 100% of spins
award 100 points.

## Fix
- Build the cumulative probability sum from ENABLED prizes only
- Renormalize so enabled prizes sum to 10000 basis points
- Disabled prizes are never selected (no fallback needed for them)
- Grand prize lock, daily cap, user cap still fall back to the configured fallback prize
- Applied to both 1X and 5X/10X paths

## What does NOT change
- Admin-configured probabilities (probability_bp values stay as-is in the snapshot)
- The fallback prize mechanism for locked/capped prizes
- 5X/10X batch logic structure
- Payment invariant, idempotency, ledger entries, structured response
*/
...
BEGIN
  ...
  -- ═══ Compute enabled prize total for renormalization ═══
  SELECT COALESCE(SUM((elem->>'probability_bp')::int), 0)
  INTO v_enabled_total_bp
  FROM jsonb_array_elements(v_version.prizes_snapshot) elem
  WHERE NOT COALESCE((elem->>'disabled')::boolean, false);

  IF v_enabled_total_bp <= 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'no_enabled_prizes');
  END IF;
  ...
  IF p_spin_count = 1 THEN
    ...
    -- Authoritative free-spin count from spin_requests
    ...
    -- ══ STEP 2: PRIZE RESOLUTION (enabled prizes only, renormalized) ══
    v_random_bucket := floor(random() * 10000)::int;
    IF v_random_bucket >= 10000 THEN v_random_bucket := 9999; END IF;

    -- Scale the random bucket to the enabled-only probability space
    v_scaled_bucket := floor((v_random_bucket::numeric * v_enabled_total_bp::numeric / 10000.0))::int;
    IF v_scaled_bucket >= v_enabled_total_bp THEN v_scaled_bucket := v_enabled_total_bp - 1; END IF;

    v_cumulative := 0;
    v_original_prize_id := NULL;
    v_prize := NULL;
    FOR v_prize IN SELECT elem FROM jsonb_array_elements(v_version.prizes_snapshot) elem
                   WHERE NOT COALESCE((elem->>'disabled')::boolean, false)
                   ORDER BY (elem->>'probability_bp')::int DESC LOOP
      v_cumulative := v_cumulative + (v_prize->>'probability_bp')::int;
      IF v_scaled_bucket < v_cumulative THEN
    ...
    -- Eligibility checks (grand lock, caps) — disabled prizes already excluded
    v_eligible := true;
    ...
    -- Write PRIZE_REWARD ledger entry
    IF v_points_awarded > 0 THEN
      INSERT INTO point_transactions (user_id, transaction_type, amount, description, reference_id, balance_before, balance_after, metadata)
  ...
  -- ═════════════════════════════════════════════════════════════════════
  -- BATCH PATH (5X/10X) — same probability fix, structure preserved
  -- ═════════════════════════════════════════════════════════════════════
