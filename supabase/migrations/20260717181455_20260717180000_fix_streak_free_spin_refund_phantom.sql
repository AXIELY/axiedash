-- Fix: Streak free-spin reward was implemented by DECREASING spins_used
-- in wheel_v2_free_spin_usage. This made free_spins_remaining positive
-- again after a paid spin completed a streak — a "phantom free spin"
-- reappeared despite the period's free spins being exhausted.
--
-- The user requirement is strict: once free spins are consumed, they
-- must NOT reappear until the period resets. Streak progress is still
-- tracked for display, but no longer refunds free spins.
...
BEGIN
  ...
  -- 5. Idempotency check — use FOUND instead of IS NOT NULL for record type
  ...
  -- If a pending/failed batch exists, delete it and its children for clean retry
  ...
  FOR v_i IN 1..p_spin_count LOOP
    ...
    -- i. Increment streak progress (tracking only — no free-spin refund)
  ...
  -- 15b. REMOVED: streak free-spin refund that decreased spins_used.
  -- Free spins are now strictly consumed and never refunded mid-period.
