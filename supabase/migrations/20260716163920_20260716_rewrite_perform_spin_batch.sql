/*
# Rewrite perform_spin_batch: Correct Batch Identity Model

Drop old function (param was p_request_id) and recreate with p_client_request_id.

## Changes
- Batch spins (5/10): create one wheel_spin_batches row, N child spin_results
  with (batch_id, sequence_number) identity — no shared spin_request_id
- Single spin (1): creates spin_requests + spin_result with standalone_request_id
- Uses wheel_progress_events ledger for idempotent progress
- reward_grants linked via spin_result_id
- Returns structured response with progress + ordered results
- Grand prize detected by type='grand', threshold from unlock_target_value
- Sequential eligibility recalculation per sub-spin
*/

DROP FUNCTION IF EXISTS public.perform_spin_batch(integer, uuid, text);
...
BEGIN
  ...
  IF v_is_batch THEN
    ...
    FOR v_i IN 1..p_spin_count LOOP
      ...
      VALUES (v_batch_id, v_user_id, v_final_prize_id, v_prize->>'type', v_prize->>'value',
      ...
      IF (v_prize->>'type') IN ('service', 'grand', 'coins') AND NOT v_fallback_used THEN
        INSERT INTO reward_grants (user_id, grant_type, grant_value, status, spin_result_id)
        VALUES (v_user_id, v_prize->>'type', COALESCE(v_prize->>'value', ''), 'pending', v_spin_result_id);
      END IF;
