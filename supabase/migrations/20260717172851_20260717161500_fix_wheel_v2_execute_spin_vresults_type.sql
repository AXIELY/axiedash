-- Fix: v_results was declared as jsonb[] (array) but is assigned a single jsonb
-- value from the SELECT COALESCE(jsonb_agg(...), '[]'::jsonb) query.
-- This caused "malformed array literal" when trying to store the JSON result.
