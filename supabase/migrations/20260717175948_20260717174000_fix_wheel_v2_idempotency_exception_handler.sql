-- Fix: The idempotency SELECT was not finding existing batches.
-- The root cause is likely a plan caching issue or the FOR UPDATE lock
-- preventing the SELECT from finding the row in the same transaction.
-- Fix: wrap the INSERT in a sub-block that catches the unique constraint
-- violation and returns the existing batch's cached results.
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- If duplicate key on the batch INSERT, the batch already exists.
    -- Fetch and return its cached results.
    IF SQLERRM LIKE '%duplicate key%' AND v_batch_id IS NULL THEN
      SELECT * INTO v_existing_batch
      FROM wheel_v2_spin_batches
      WHERE user_id = v_user_id AND client_request_id = p_client_request_id;
