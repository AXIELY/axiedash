...
/*
# Fix perform_spin: Economy Safety Controls

## Problem
1. Disabled prizes (weight=0 or disabled=true) were not excluded — a prize with weight 0
   would never be selected by the weighted walk but could still be picked as the fallback.
2. Fallback picked the LAST prize in the array — dangerous if that's the grand prize.
3. No daily winner cap enforcement (max_winners_per_day was stored but not checked).
4. No internal cost cap enforcement.

## Changes
1. Add disabled/weight-zero check to the eligibility filter (both passes).
2. Change fallback to select the cheapest eligible prize (lowest internal_cost_estimate),
   defaulting to the first points-type prize, never a service/grand/coins prize.
3. Add daily winner cap check: count today's wins for each prize and skip if exceeded.
4. Abort spin (refund) if NO eligible prizes remain after filtering.

## Modified Functions
- perform_spin (batch version, latest OID) — complete replacement.
...
## Important Notes
1. The old single-spin perform_spin (older OID) is also dropped to avoid confusion.
2. All existing spin results and rewards are preserved.
3. The function signature remains identical: perform_spin(p_client_request_id uuid, p_quantity int).
*/

-- Drop ALL existing versions
DROP FUNCTION IF EXISTS perform_spin(uuid);
DROP FUNCTION IF EXISTS perform_spin(uuid, int);

CREATE OR REPLACE FUNCTION perform_spin(p_client_request_id uuid, p_quantity int DEFAULT 1)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_eligible        boolean;
  v_pts             int;
  v_daily_wins_for_prize int;
  v_fallback_prize  jsonb := NULL;
  v_fallback_index  int := 0;
...
BEGIN
  ...
  FOR v_batch_index IN 1..v_quantity LOOP
    ...
    -- ── Eligibility pass 1: accumulate total weight ──────────────────────
    v_total_weight := 0;
    v_fallback_prize := NULL;
    ...
    FOR v_prize_index IN 0..jsonb_array_length(v_prizes) - 1 LOOP
      ...
      -- NEW: Skip disabled prizes (weight=0 or disabled flag)
      IF COALESCE((v_candidate->>'disabled')::boolean, false) THEN
        v_eligible := false;
      END IF;
      IF v_eligible AND COALESCE((v_candidate->>'weight')::numeric, 0) <= 0 THEN
        v_eligible := false;
      END IF;

      -- Strong prize already won by user
      IF v_eligible AND COALESCE((v_candidate->>'is_strong')::boolean, false)
      AND (v_candidate->>'id') = ANY(v_won_strong_ids) THEN
        v_eligible := false;
      END IF;

      -- LOCKED_BY_GOAL / EVENT_ONLY
      ...
      -- NEW: Daily winner cap
      IF v_eligible AND (v_candidate->>'max_winners_per_day') IS NOT NULL THEN
        SELECT COUNT(*) INTO v_daily_wins_for_prize
        FROM spin_results
        WHERE prize_id = v_candidate->>'id'
        AND created_at >= date_trunc('day', v_now);
        IF v_daily_wins_for_prize >= (v_candidate->>'max_winners_per_day')::int THEN
          v_eligible := false;
        END IF;
      END IF;
      ...
      IF v_eligible THEN
        ...
        -- Track safest fallback: prefer points type with lowest cost
        IF v_fallback_prize IS NULL
        OR (v_candidate->>'type' = 'points' AND COALESCE(v_fallback_prize->>'type','') != 'points')
        OR (v_candidate->>'type' = 'points' AND v_fallback_prize->>'type' = 'points'
            AND COALESCE((v_candidate->>'internal_cost_estimate')::numeric, 0)
              < COALESCE((v_fallback_prize->>'internal_cost_estimate')::numeric, 0))
        THEN
          v_fallback_prize := v_candidate;
          v_fallback_index := v_prize_index;
        END IF;
      END IF;
    END LOOP;

    -- SAFETY: If no eligible prizes, abort entire batch and refund
    IF NOT v_has_eligible OR v_total_weight <= 0 THEN
      -- Refund points
      IF v_points_deducted > 0 THEN
        UPDATE users SET points = points + v_points_deducted WHERE id = v_user_id;
      END IF;
      UPDATE spin_requests SET status = 'failed' WHERE id = v_spin_req_id;
      RETURN jsonb_build_object('success', false, 'error', 'no_eligible_prizes', 'refunded', true);
    END IF;

    -- ── Eligibility pass 2: weighted selection ───────────────────────────
    ...
    FOR v_prize_index IN 0..jsonb_array_length(v_prizes) - 1 LOOP
      ...
      -- Exact same eligibility checks as pass 1
      IF COALESCE((v_candidate->>'disabled')::boolean, false) THEN v_eligible := false; END IF;
      IF v_eligible AND COALESCE((v_candidate->>'weight')::numeric, 0) <= 0 THEN v_eligible := false; END IF;

      IF v_eligible AND COALESCE((v_candidate->>'is_strong')::boolean, false)
      AND (v_candidate->>'id') = ANY(v_won_strong_ids) THEN v_eligible := false; END IF;
      ...
      IF v_eligible AND v_avail_mode IN ('LOCKED_BY_GOAL', 'EVENT_ONLY') THEN
        ...
        IF NOT FOUND OR v_prize_state.runtime_status != 'ACTIVE' THEN
          IF v_avail_mode = 'LOCKED_BY_GOAL' THEN
            ...
            WHERE user_id = v_user_id AND prize_id = v_candidate->>'id' AND settings_id = v_settings.id;
            IF v_user_unlocked IS NULL THEN v_eligible := false; END IF;
          ELSE v_eligible := false; END IF;
        END IF;
      END IF;

      IF v_eligible AND v_avail_mode = 'SCHEDULED' THEN
        IF v_now < COALESCE((v_candidate->>'starts_at')::timestamptz, '2000-01-01'::timestamptz)
        OR v_now > COALESCE((v_candidate->>'ends_at')::timestamptz, '2999-12-31'::timestamptz) THEN v_eligible := false; END IF;
      END IF;

      IF v_eligible AND v_avail_mode = 'LIMITED_STOCK' THEN
        SELECT available_stock, runtime_status INTO v_prize_state.available_stock, v_prize_state.runtime_status
        FROM wheel_prize_states WHERE prize_id = v_candidate->>'id' AND settings_id = v_settings.id;
        IF FOUND AND (v_prize_state.runtime_status = 'EXHAUSTED' OR COALESCE(v_prize_state.available_stock, 0) <= 0) THEN v_eligible := false; END IF;
      END IF;
      ...
      IF v_eligible AND v_avail_mode = 'LIMITED_WINNERS' THEN
        ...
        FROM wheel_prize_states WHERE prize_id = v_candidate->>'id' AND settings_id = v_settings.id;
        IF FOUND AND (v_prize_state.runtime_status = 'EXHAUSTED'
        OR v_prize_state.winners_count >= COALESCE((v_candidate->>'max_winners')::int, 999999)) THEN v_eligible := false; END IF;
      END IF;

      IF v_eligible AND v_avail_mode = 'ALWAYS_ACTIVE' THEN
        IF (v_candidate->>'starts_at') IS NOT NULL AND v_now < (v_candidate->>'starts_at')::timestamptz THEN v_eligible := false; END IF;
        IF v_eligible AND (v_candidate->>'ends_at') IS NOT NULL AND v_now > (v_candidate->>'ends_at')::timestamptz THEN v_eligible := false; END IF;
      END IF;
      ...
      IF v_eligible AND (v_candidate->>'user_cooldown_days') IS NOT NULL THEN
        ...
        WHERE sr.user_id = v_user_id AND sr.prize_id = v_candidate->>'id';
        IF v_last_win_at IS NOT NULL
        AND v_now < v_last_win_at + ((v_candidate->>'user_cooldown_days')::int * interval '1 day') THEN v_eligible := false; END IF;
      END IF;

      IF v_eligible AND (v_candidate->>'max_winners_per_day') IS NOT NULL THEN
        SELECT COUNT(*) INTO v_daily_wins_for_prize FROM spin_results
        WHERE prize_id = v_candidate->>'id' AND created_at >= date_trunc('day', v_now);
        IF v_daily_wins_for_prize >= (v_candidate->>'max_winners_per_day')::int THEN v_eligible := false; END IF;
      END IF;
    ...
    -- SAFE FALLBACK: use cheapest eligible points prize, never grand/service
    IF v_prize IS NULL THEN
      v_prize := v_fallback_prize;
      v_prize_index := v_fallback_index;
    ELSE
      FOR v_prize_index IN 0..jsonb_array_length(v_prizes) - 1 LOOP
