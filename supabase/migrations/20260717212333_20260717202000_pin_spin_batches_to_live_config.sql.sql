/*
# Pin spin batches to live config revision + checksum

## Purpose
Adds `live_config_revision` and `live_config_checksum` columns to
`wheel_v2_spin_batches` so every batch is pinned to the exact configuration
that was loaded at the start of the request. This is a transaction
consistency token — NOT a version management workflow.

If the Admin changes settings while a 10X batch is running:
- the current batch completes using its pinned revision/checksum
- the next new request uses the new live configuration

## Changes
- wheel_v2_spin_batches: + live_config_revision integer, + live_config_checksum text
- execute_wheel_spins: updated to read revision+checksum from wheel_live_settings
  and store them on the batch row

## Notes
1. The spin engine already loads config from wheel_v2_config_versions via
   get_published_wheel_v2(). Since save_wheel_live_config() mirrors to that
   table and updates the runtime pointer, the engine continues to work
   unchanged. We only ADD the pinning columns for audit/consistency.
2. No existing data is lost — new columns are nullable.
*/

-- Add pinning columns
ALTER TABLE wheel_v2_spin_batches
  ADD COLUMN IF NOT EXISTS live_config_revision integer;

ALTER TABLE wheel_v2_spin_batches
  ADD COLUMN IF NOT EXISTS live_config_checksum text;

-- Update execute_wheel_spins to pin the batch to the live config revision+checksum
...
BEGIN
  ...
  -- 3. Load current PUBLISHED configuration (mirrored from live config)
  ...
  -- 3b. Pin to live config revision+checksum for transaction consistency
  SELECT revision, config_checksum INTO v_live_revision, v_live_checksum
  FROM wheel_live_settings WHERE id = 1;
  ...
  IF v_batch_found AND v_existing_batch.status = 'completed' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'live_config_revision', v_existing_batch.live_config_revision,
      'live_config_checksum', v_existing_batch.live_config_checksum,
  ...
  IF v_batch_found AND v_existing_batch.status IN ('pending', 'failed') THEN
  ...
  SELECT points INTO v_points_before FROM users WHERE id = v_user_id FOR UPDATE;
  ...
  -- 10. Create parent batch (pinned to live config revision+checksum)
  ...
  INSERT INTO wheel_v2_spin_batches (
    ...
    live_config_revision, live_config_checksum
  ) VALUES (
    ...
    'pending', 0,
    v_live_revision, v_live_checksum
  )
  ...
  IF v_total_cost > 0 THEN
    ...
    INSERT INTO point_transactions (
      ...
      jsonb_build_object('game', 'wheel_v2', 'spin_count', p_spin_count, 'paid_spins', v_paid_count, 'free_spins', v_free_used, 'live_config_revision', v_live_revision)
    );
  END IF;
  ...
  FOR v_i IN 1..p_spin_count LOOP
    ...
    IF NOT FOUND THEN
      UPDATE wheel_v2_spin_batches SET status = 'failed', failure_code = 'NO_PRIZE_SELECTED', completed_at = now() WHERE id = v_batch_id;
    ...
    IF v_prize.is_grand_prize AND v_is_grand_prize_locked THEN
      ...
      IF v_prize.fallback_prize_key IS NOT NULL THEN
        ...
        WHERE prize_key = v_prize.fallback_prize_key AND enabled = true LIMIT 1;
        v_final_prize_key := v_prize.prize_key;
      ELSE
        UPDATE wheel_v2_spin_batches SET status = 'failed', failure_code = 'PRIZE_FALLBACK_UNAVAILABLE', completed_at = now() WHERE id = v_batch_id;
  ...
  IF v_points_reward > 0 THEN
    ...
    INSERT INTO point_transactions (
      ...
      jsonb_build_object('game', 'wheel_v2', 'spin_count', p_spin_count, 'live_config_revision', v_live_revision)
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'live_config_revision', v_live_revision,
    'live_config_checksum', v_live_checksum,
    'requested_spin_count', p_spin_count,
    'payment', jsonb_build_object(
      'free_spins_before', v_free_remaining,
      'free_spins_used', v_free_used,
      'free_spins_after', GREATEST(0, v_free_remaining - v_free_used),
