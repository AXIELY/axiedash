-- Rewrite perform_spin_batch to:
-- 1. Compute cost as single_spin_cost × spin_count (no hardcoded discounts)
-- 2. Check multi_spin_enabled flag for batch spins
-- 3. Store single_spin_cost on the batch row
-- 4. Return structured response with balance_before/after and progress
-- 5. Use batch_id + sequence_number for child identity (already correct in schema)
...
BEGIN
  ...
  IF v_is_batch THEN
    ...
    IF FOUND THEN
      IF v_existing_batch.status = 'COMPLETED' THEN
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'prize_index',
            (SELECT idx - 1 FROM (
              ...
              FROM jsonb_array_elements(v_existing_batch.prizes_snapshot) elem
        ...
        -- Resolve grand prize info for progress display
        ...
        'prize_index',
          (SELECT idx - 1 FROM (
            ...
            FROM jsonb_array_elements(
              (SELECT prizes FROM wheel_game_settings WHERE active = true LIMIT 1)
            ) elem
          ) sub WHERE sub.pid = sr.final_awarded_prize_id LIMIT 1)
      )) INTO v_existing_results
      FROM spin_results sr
  ...
  -- EXACT COST: single_spin_cost × spin_count (no discounts)
  v_single_cost := v_settings.single_spin_cost;

  IF p_spin_count = 1 THEN
    -- Check free spins for 1x
  ...
  IF v_is_batch THEN
    ...
    FOR v_i IN 1..p_spin_count LOOP
      ...
      -- Same probability engine as 1x: weighted random selection from published snapshot
      v_cumulative := 0;
      ...
      -- Insert child result with batch_id + sequence_number identity
      ...
      -- Create reward grant for service/grand/coins prizes
      ...
      -- Progress event (one per result, protected by unique constraint)
      ...
      -- Build result entry with prize_index
  ...
  VALUES (v_user_id, p_client_request_id, v_version.id,
    CASE WHEN v_use_free THEN 'free' ELSE 'paid' END, v_total_cost, 'completed')
  ON CONFLICT (user_id, client_request_id) DO NOTHING
  RETURNING id INTO v_spin_request_id;
