-- Fix: execute_wheel_spins fallback prize lookup was using raw table
-- which lacks range_start/range_end columns, causing TRANSACTION_FAILED
-- whenever a fallback was triggered (grand prize locked or daily win limit).
-- Now uses build_wheel_v2_probability_ranges() for consistent record shape
-- and saves range_start/range_end before fallback overwrite.
...
DECLARE
  ...
  -- Saved probability range (before fallback overwrites v_prize)
...
BEGIN
  ...
  FOR v_i IN 1..p_spin_count LOOP
    ...
    IF v_prize.daily_win_limit IS NOT NULL AND v_prize.daily_win_limit > 0 THEN
      ...
      IF v_daily_win_count >= v_prize.daily_win_limit THEN
        ...
        IF v_prize.fallback_prize_key IS NOT NULL AND v_prize.fallback_prize_key != '' THEN
          ...
          v_final_prize_key := v_prize.prize_key;
        ELSE
          -- Skip this prize, award NO_REWARD
    ...
    -- e. Store child result (use saved range values from original draw)
