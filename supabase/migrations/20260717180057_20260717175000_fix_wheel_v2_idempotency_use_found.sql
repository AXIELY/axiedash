-- Fix: The idempotency check used `v_existing_batch IS NOT NULL` which
-- returns false for record types when any column is NULL.
-- Use the FOUND variable instead, which is set by SELECT ... INTO.
...
DROP FUNCTION IF EXISTS public.debug_execute_wheel(text);
